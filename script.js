...
// ── Scroll canvas — beauty-grooming, 151 frame, cover mode ──
(function () {
  'use strict';
  ...
  var FRAME_PATH   = 'https://8ispuxmgjxgu2r5q.public.blob.vercel-storage.com/templates/estetica-001/frames/';
...
// ── IntersectionObservers — fade-up, stagger, fade-side ──
if ('IntersectionObserver' in window) {
  ...
  document.querySelectorAll('.fade-side').forEach(function (el) { sideObs.observe(el); });

} else {
  document.querySelectorAll('.fade-up, .stagger-card, .fade-side').forEach(function (el) {
...
window.addEventListener('load', function () {
  setTimeout(function () {
    document.querySelectorAll('.fade-up:not(.visible), .stagger-card:not(.visible), .fade-side:not(.visible)').forEach(function (el) {
...
window.__imgFallback = function (img, label) {
  ...
  wrap.innerHTML =
    ...
    '<text x="400" y="310" text-anchor="middle" font-family="DM Serif Display, serif" font-style="italic" font-size="18" fill="currentColor" opacity="0.25">' + label + '</text>' +
    '</svg>';
